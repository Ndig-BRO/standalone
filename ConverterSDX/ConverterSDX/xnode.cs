...
namespace xnode
{
    ...
    public class XMLNode : MarshalByRefObject
    {
        ...
        {
            string mimeStr = Convert.ToBase64String( data, 0, data.Length, Base64FormattingOptions.InsertLineBreaks );
        ...
        public string getText( bool bDecode = false )
        {
            ...
            if ( bDecode || getAttr( "Encoded", 0 ) != 0 )
            {
                ...
                {
                    Console.WriteLine( "Base 64 string length is not " + "4 or is not an even multiple of 4." );
                }
            }
            return text;
        }
        ...
        public byte[] getData( bool bDecode = false )
        {
            ...
            if ( bDecode || getAttr( "Encoded", 0 ) != 0 )
            {
                try
                {
                    binaryData = Convert.FromBase64String( text );
                }
                catch ( ArgumentNullException )
                {
                    Console.WriteLine( "Base 64 string is null." );
